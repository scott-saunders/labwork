...
```{r}
df_1 <- read_csv("data/plateReaderToxicity_hr.csv")
...
fit_gompertz_mod <- function(df){
  ...
  mod <- nls(formula=absorbance~A*exp(-exp((mu/A)*(lam - time) + 1))+c, start = c(A = 1, mu = 0.2, lam = 3, c = 0.1), data = df)
...
p <- ggplot(data = df_preds, aes_string(x=df_preds$time, y = df_preds$absorbance)) + 
  geom_point() + geom_path(aes(y = df_preds$pred), color = 'blue')+ facet_wrap(~strain)
