...
blank_estimates_og <- blank_estimates %>% 
  filter(model=='og') %>% 
  mutate(dm_soak = dm_from_a_OG(estimate = estimate, i_0 = i0_soak, dap = 1e-5, t_s = 0.1)) %>% 
  mutate(dm_transfer = dm_from_a_OG(estimate = estimate, i_0 = i0_transfer, dap = 1e-5, t_s = 0.1))

blank_estimates_noFlux <- blank_estimates %>% 
  filter(model=='no_flux') %>% 
  mutate(dm_soak = dm_from_a_noFlux(estimate = estimate, i_0 = i0_soak, dap = 1e-5, t_s = 0.1)) %>% 
  mutate(dm_transfer = dm_from_a_noFlux(estimate = estimate, i_0 = i0_transfer, dap = 1e-5, t_s = 0.1)) 
...
All this is to say, that I do not know how to theoretically get at the scan time, and as far as I know now anything between 0.3 and 0.001 seconds could probably be justified...See below for how the blank data look when we use an empirically "good" value of $t_s = 0.02$:
```{r}
blank_estimates_og <- blank_estimates %>% 
  filter(model=='og') %>% 
  mutate(dm_soak = dm_from_a_OG(estimate = estimate, i_0 = i0_soak, dap = 1e-5, t_s = 0.02)) %>% 
  mutate(dm_transfer = dm_from_a_OG(estimate = estimate, i_0 = i0_transfer, dap = 1e-5, t_s = 0.02))

blank_estimates_noFlux <- blank_estimates %>% 
  filter(model=='no_flux') %>% 
  mutate(dm_soak = dm_from_a_noFlux(estimate = estimate, i_0 = i0_soak, dap = 1e-5, t_s = 0.02)) %>% 
  mutate(dm_transfer = dm_from_a_noFlux(estimate = estimate, i_0 = i0_transfer, dap = 1e-5, t_s = 0.02)) 
...
ggplot(blank_estimates_tidy, aes(x = model, shape = PHZadded)) + 
  ...
  scale_y_log10()
```
Now both models have the expected behavior. The no flux/finite model estimates straddle the given $D_{ap}$, where the soak $I_0$ estimates are true overestimates and the transfer $I_0$ estimates are true underestimates. Further, the og model now significantly underestimates $D_m$ as would be expected from the poor fit of the data. 
...
dm_estimates_og <- dm_estimates %>% 
  filter(model=='og') %>% 
  mutate(dm_soak = dm_from_a_OG(estimate = estimate, i_0 = i0_soak, dap = 1e-6, t_s = 0.02)) %>% 
  mutate(dm_transfer = dm_from_a_OG(estimate = estimate, i_0 = i0_transfer, dap = 1e-6, t_s = 0.02))

dm_estimates_noFlux <- dm_estimates %>% 
  filter(model=='no_flux') %>% 
  mutate(dm_soak = dm_from_a_noFlux(estimate = estimate, i_0 = i0_soak, dap = 1e-6, t_s = 0.02)) %>% 
  mutate(dm_transfer = dm_from_a_noFlux(estimate = estimate, i_0 = i0_transfer, dap = 1e-6, t_s = 0.02)) 
...
ggplot(dm_estimates_tidy, aes(x = model, shape = electrode)) + 
  ...
  scale_y_log10(limits = c(1e-9,1e-6))
```
By using this 0.02 value for $t_s$ you can see that the estimates for the biofilm $D_m$ shift downward. If this process is defensible then we may have a case that $D_m$ could be slower than $D_{ap}$ in the biofilm.
...
3. **The issue of $t_s$** 
  ...
  * If we assume for the blank IDA that $D_m \approx D_{ap}$, then empirically $t_s \approx 0.02 \text{ sec}$ provides a good estimate. **I am not sure if this will be considered legit enough...** 
...
Moving forward, let's get feedback on estimating $t_s$, and we should probably talk to someone about more clearly/rigorously deriving the finite diffusion model.
