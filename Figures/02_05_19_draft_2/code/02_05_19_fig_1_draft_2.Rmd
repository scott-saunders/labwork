...
plot_wt_free <- ggplot(wt_conc_means, aes(x = material)) + 
  ...
  geom_jitter(aes(y = phz_conc), height = 0, width = 0.1, shape = 21,color = 'black', size = 2) +
...
plot_dphz_binding <- ggplot(dphz_extracts_means, aes(x=added_phz_num,y=calcConc))+
  geom_line(size=1, aes(y=mean, group = measured_phenazine))+
  geom_point(size=2, aes(shape=measured_phenazine))
...
# Combine Panels 
```{r}
top_panel <- plot_grid(NULL, wt_ret_ratio_styled, ncol = 2, rel_widths = c(2.5,1), scale = 0.9, labels = c('',"E"))

bottom_panel <- plot_grid(plot_wt_free_styled, plot_dphz_binding_styled, 
                   ncol = 2,
                   align = 'hv', axis = 'tblr', 
                   rel_widths = c(1.25,1), 
                   scale = 0.9, labels = c("D", "F"))
