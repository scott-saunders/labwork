...
```{r}
ggplot(df_cal, aes(x = signal_from_swv, y = signal_from_gc)) + geom_point() + geom_smooth(method='lm')
...
Here's the data:
```{r}
ggplot(df_cal, aes(x = C_mol_cm3, y = signal_from_swv)) + geom_point() + geom_smooth(method='lm')
```
...
```{r}
ggplot(df_cal, aes(x = C_mol_cm3, y = signal_from_gc)) + geom_point() + geom_smooth(method='lm')
```
Again, very linear.
...
Here it is plotted:
```{r}
ggplot(lms, aes(x = dataset, y = dap)) + 
